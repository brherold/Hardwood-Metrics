...
SELECT 
    ...
    -- Total field goal makes against
    SUM(def.F_M) + SUM(def.IS_M) + SUM(def.MR_M) + SUM(def._3P_M) AS FG_M,	
    ...
    END AS PPP,
	 ...
	 -- Total 2P  makes against
    SUM(def.F_M) + SUM(def.IS_M) + SUM(def.MR_M)  AS _2P_M,	
    
    -- Total 2P  attempts against
	...
	-- 2P Percentage allowed as decimal float
	CASE 
		WHEN (SUM(def.F_A) + SUM(def.IS_A) + SUM(def.MR_A)) > 0 THEN 
			ROUND(
