...
SELECT 
    def.defense_type,
	COUNT(CASE WHEN def.plays >= 1 THEN 1 END) AS G_P,
	...
	SUM(def.F_M) + SUM(def.IS_M) + SUM(def.MR_M) AS _2P_M,

	-- 2-point attempts (sum of all 2-point attempts) 
	SUM(def.F_A) + SUM(def.IS_A) + SUM(def.MR_A) AS _2P_A,

	-- 2-point percentage (makes/attempts)
	CASE 
		WHEN SUM(def.F_A) + SUM(def.IS_A) + SUM(def.MR_A) > 0 
		THEN ROUND(
			CAST(SUM(def.F_M) + SUM(def.IS_M) + SUM(def.MR_M) AS FLOAT) / 
			CAST(SUM(def.F_A) + SUM(def.IS_A) + SUM(def.MR_A) AS FLOAT),
			3
		)
		ELSE 0.0 
	END AS _2P_P,
