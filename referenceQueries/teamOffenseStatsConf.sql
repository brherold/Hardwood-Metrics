WITH total_plays_all AS (
    ...
    WHERE off.team_id = ? 
      AND g.season_id = ? 
      AND g.game_type in ("Non-Conference", "Conference", "Playoff","Tournament")
)
SELECT 
    off.defense_type,
	COUNT(CASE WHEN off.plays >= 1 THEN 1 END) AS G_P,
	...
	-- 2-point makes (sum of all 2-point buckets)
	SUM(off.F_M) + SUM(off.IS_M) + SUM(off.MR_M) AS _2P_M,

	-- 2-point attempts (sum of all 2-point attempts) 
	SUM(off.F_A) + SUM(off.IS_A) + SUM(off.MR_A) AS _2P_A,

	-- 2-point percentage (makes/attempts)
	CASE 
		WHEN SUM(off.F_A) + SUM(off.IS_A) + SUM(off.MR_A) > 0 
		THEN ROUND(
			CAST(SUM(off.F_M) + SUM(off.IS_M) + SUM(off.MR_M) AS FLOAT) / 
			CAST(SUM(off.F_A) + SUM(off.IS_A) + SUM(off.MR_A) AS FLOAT),
			3
		)
		ELSE 0.0 
	END AS _2P_P,
